...
class HockeyApplication:    
    ...
    def add_players(self, allplayers: list):
    ...
    def scored_points(self, player: Player):
    ...
    def list_players_in_team(self):
        ...
        team_players = sorted(team_players, key=self.scored_points, reverse=True)
    ...
    def list_players_in_country(self):
        ...
        country_players = sorted(country_players, key=self.scored_points, reverse=True)
        for player in country_players:
            print(player)

    def list_players_most_points(self):
        max_num_of_players = int(input('how many: '))
        points_players = sorted(self.players, key=self.scored_points, reverse=True)
        count = 1
    ...
    def scored_goals(self, player: Player):
        return [player.goals, (player.games)*-1]

    def list_players_most_goals(self):
        max_num_of_players = int(input('how many: '))
        points_players = sorted(self.players, key=self.scored_goals, reverse=True)
        count = 1
        for player in points_players:
            if count <= max_num_of_players:
                print(player)
